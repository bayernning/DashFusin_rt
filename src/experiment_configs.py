...
EXPERIMENT_CONFIGS = [
    ...
    
    {
        ...
        'params': {
            'learning_rate': 2e-3,  # ← 大batch需要更大lr
            'batch_size': 60,       # ← 增大
            'epochs': 100,
            'num_bottleneck': 8,
            'num_fusion_layers': 2,
    ...
    
    {
        ...
        'params': {
            'learning_rate': 5e-4,   # ← 小lr
            'batch_size': 16,        # ← 小batch
            'epochs': 100,
            'num_bottleneck': 8,
            'num_fusion_layers': 2,
            'contrast_loss_weight': 0.05,  # ← 弱对比
            'temperature': 0.2,      # ← 高温度
            'dropout': 0.5,          # ← 强dropout
            'warmup_steps': 30,
    ...
    
    {
        ...
        'params': {
            'learning_rate': 2e-3,   # ← 大lr
            'batch_size': 60,        # ← 大batch
            'epochs': 100,
            'num_bottleneck': 16,    # ← 大容量
            'num_fusion_layers': 3,  # ← 深层
            'contrast_loss_weight': 0.2,  # ← 强对比
            'temperature': 0.1,
